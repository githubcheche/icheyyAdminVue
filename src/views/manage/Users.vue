    ...
    <div class="table">
      <el-table v-bind:data="tableData" border style="width: 100%" highlight-current-row :fit="listWidth">
        ...
        <el-table-column prop="is_confirmed" sortable label="是否激活：" :align="align">
          <template slot-scope="scope">
            <el-input v-show="scope.row.edit" size="small" v-model="scope.row.is_confirmed"/>
            <span v-show="!scope.row.edit">{{ scope.row.is_confirmed }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="is_banned" sortable label="是否禁用：" :align="align">
          <template slot-scope="scope">
            <el-input v-show="scope.row.edit" size="small" v-model="scope.row.is_banned"/>
        ...
        <el-table-column label="操作" width="220" :align="align">
          <template slot-scope="scope">
            <el-button :type="scope.row.edit?'success':'primary'"
                       @click='handerUpdate(scope.$index, scope.row.id, scope.row.edit=!scope.row.edit)' size="small"
      ...
      <el-dialog :title="dialogTitle" v-bind:close-on-click-modal="false" v-model="showEdit"
      ...
      <div class="pagination">
        <el-pagination layout="sizes,prev, pager, next" @current-change="handleCurrentChange"
                       @size-change="handleSizeChange" :total="total" :page-size="pageSize" :page-sizes="pageSizes">
        </el-pagination>
      </div>
    </div>
  </div>
